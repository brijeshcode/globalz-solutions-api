...
namespace Tests\Feature\Customers\Payments\Concerns;
...
trait HasCustomerPaymentSetup
{
    ...
    public function setUpCustomerPayments(): void
    {
    ...
    protected function createPaymentViaApi(array $overrides = [], string $as = 'admin'): CustomerPayment
    {
        $user = $as === 'admin' ? $this->admin : $this->salesman;
        $this->actingAs($user, 'sanctum');

        $this->postJson(route('customers.payments.store'), $this->paymentPayload($overrides))
            ->assertCreated();

        return CustomerPayment::latest()->first();
    }

    protected function createPaymentViaFactory(?string $state = null, array $overrides = []): CustomerPayment
    {
        $base = [
            'customer_id'              => $this->customer->id,
            'currency_id'              => $this->currency->id,
