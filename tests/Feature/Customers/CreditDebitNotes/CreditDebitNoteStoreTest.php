<?php

use App\Models\Customers\Customer;
use App\Models\Customers\CustomerCreditDebitNote;
use Tests\Feature\Customers\CreditDebitNotes\Concerns\HasCreditDebitNoteSetup;

uses(HasCreditDebitNoteSetup::class);

beforeEach(fn () => $this->setUpCreditDebitNotes());

it('creates a credit note', function () {
    $this->postJson(route('customers.credit-debit-notes.store'), $this->creditNotePayload())
        ->assertCreated()
        ->assertJsonStructure(['message', 'data' => ['id', 'code', 'type', 'customer', 'currency']])
        ->assertJsonPath('data.type', 'credit');

    $this->assertDatabaseHas('customer_credit_debit_notes', [
        'customer_id' => $this->customer->id,
        'type'        => 'credit',
        'prefix'      => 'CRN',
    ]);
});

it('creates a debit note', function () {
    $this->postJson(route('customers.credit-debit-notes.store'), $this->debitNotePayload())
        ->assertCreated()
        ->assertJsonPath('data.type', 'debit');

    $this->assertDatabaseHas('customer_credit_debit_notes', [
        'customer_id' => $this->customer->id,
        'type'        => 'debit',
        'prefix'      => 'DBN',
    ]);
});

it('sets created_by and updated_by to the authenticated user', function () {
    $note = $this->createNoteViaApi();

    expect($note->created_by)->toBe($this->admin->id)
        ->and($note->updated_by)->toBe($this->admin->id);
});

it('auto-generates a 6-digit note code', function () {
    $note = $this->createNoteViaApi();

    expect($note->code)->not()->toBeNull()
        ->and($note->code)->toMatch('/^\d{6}$/');
});

it('rejects a salesman', function () {
    $this->actingAs($this->salesman, 'sanctum');

    $this->postJson(route('customers.credit-debit-notes.store'), $this->creditNotePayload())
        ->assertForbidden();
});

it('requires all mandatory fields', function () {
    $this->postJson(route('customers.credit-debit-notes.store'), [
        'customer_id'   => null,
        'currency_id'   => null,
        'type'          => 'invalid',
        'prefix'        => 'INVALID',
        'amount'        => -100,
        'currency_rate' => 0,
    ])->assertUnprocessable()
      ->assertJsonValidationErrors(['date', 'customer_id', 'currency_id', 'type', 'prefix', 'amount', 'currency_rate']);
});

it('rejects a debit prefix for a credit note', function () {
    $this->postJson(route('customers.credit-debit-notes.store'),
        $this->creditNotePayload(['type' => 'credit', 'prefix' => 'DBN'])
    )->assertUnprocessable()
     ->assertJsonValidationErrors(['prefix']);
});

it('rejects a credit prefix for a debit note', function () {
    $this->postJson(route('customers.credit-debit-notes.store'),
        $this->debitNotePayload(['type' => 'debit', 'prefix' => 'CRN'])
    )->assertUnprocessable()
     ->assertJsonValidationErrors(['prefix']);
});

it('rejects amount_usd that does not match currency rate calculation', function () {
    $this->postJson(route('customers.credit-debit-notes.store'),
        $this->creditNotePayload(['amount' => 100.00, 'amount_usd' => 200.00, 'currency_rate' => 1.25])
    )->assertUnprocessable()
     ->assertJsonValidationErrors(['amount_usd']);
});

it('rejects an inactive customer', function () {
    $inactive = Customer::factory()->create(['is_active' => false]);

    $this->postJson(route('customers.credit-debit-notes.store'),
        $this->creditNotePayload(['customer_id' => $inactive->id])
    )->assertUnprocessable()
     ->assertJsonValidationErrors(['customer_id']);
});
