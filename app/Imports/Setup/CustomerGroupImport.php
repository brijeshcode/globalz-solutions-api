...
class CustomerGroupImport implements ToCollection, WithHeadingRow, WithBatchInserts, WithChunkReading
{
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $index => $row) {
            ...
            try {
                ...
                $existingGroup = CustomerGroup::withTrashed()
                ...
                if ($existingGroup) {
                    if ($this->updateExisting) {
                        ...
                        $existingGroup->update($validatedData);
                        ...
                        $this->results['errors'][] = [
                            'row' => $rowNumber,
                            'error' => "Customer group already exists with name: {$validatedData['name']}"
                        ];
                    }
                } else {
                ...
                Log::error("CustomerGroup Row {$rowNumber} processing failed", [
                    'error' => $e->getMessage(),
                    'file' => $e->getFile(),
                    'line' => $e->getLine()
                ]);
    ...
    protected function validateRow(array $row, int $rowNumber): array
    {
        ...
        if (empty($data['name'])) {
            return ['error' => 'Customer group name is required'];
        }
