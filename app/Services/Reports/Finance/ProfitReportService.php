...
/**
 * Single source of truth for the monthly / net profit calculation.
 *
 * Both the Monthly Profit report and the Balance Sheet's "Retained Earnings"
 * (accumulated lifetime profit) line derive from here, so the final_net_profit
 * formula is defined in exactly one place and cannot drift between the two.
 */
class ProfitReportService
{
    /**
     * Full monthly profit breakdown for a single year (12 months + year totals).
     */
    public function getMonthlyProfitData(int $year): array
    {
        ...
        $returnsData = DB::table('customer_return_items')
        ...
        $expensesData = DB::table('expense_transactions')
        ...
        $salariesData = DB::table('salaries')
            ->selectRaw('
                month,
                year,
        ...
        $creditNotesData = DB::table('customer_credit_debit_notes')
            ->selectRaw('
                MONTH(date) as month,
                YEAR(date) as year,
    ...
    /**
     * Accumulated lifetime net profit: the sum of every year's final_net_profit.
     * Reuses getMonthlyProfitData() per year so the formula stays defined once.
     */
    public function getLifetimeNetProfit(): float
    {
        $total = 0.0;

        foreach ($this->getYearsWithData() as $year) {
            $total += $this->getMonthlyProfitData($year)['year_totals']['final_net_profit'];
        }
    ...
    /**
     * Every distinct year that has data in any of the profit source tables.
     * Used to bound the lifetime accumulation loop.
     *
     * @return int[]
     */
    private function getYearsWithData(): array
    {
        $years = collect();

        $years = $years
            ->merge(DB::table('sales')->whereNull('deleted_at')->selectRaw('DISTINCT YEAR(date) as y')->pluck('y'))
            ->merge(DB::table('customer_returns')->whereNull('deleted_at')->selectRaw('DISTINCT YEAR(date) as y')->pluck('y'))
            ->merge(DB::table('expense_transactions')->whereNull('deleted_at')->selectRaw('DISTINCT YEAR(expense_month) as y')->pluck('y'))
            ->merge(DB::table('salaries')->whereNull('deleted_at')->selectRaw('DISTINCT year as y')->pluck('y'))
            ->merge(DB::table('customer_credit_debit_notes')->whereNull('deleted_at')->selectRaw('DISTINCT YEAR(date) as y')->pluck('y'));
